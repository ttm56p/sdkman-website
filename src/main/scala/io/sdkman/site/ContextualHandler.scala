...
class ContextualHandler extends Handler with Configuration {
  ...
  override def handles(implicit ctx: Context): Unit = {
    Option(ctx.getAllPathTokens.get("context")) match {
      case Some("index") => OK(html.index(optionsEnabled, recaptchaEnabled, recaptchaSiteKey))
      ...
      case None => OK(html.index(optionsEnabled, recaptchaEnabled, recaptchaSiteKey))
    }
  }
}
